...
# Configuration
DATASET_PATH = "C:\\Users\\Aarav\\Desktop\\cheff\\smart-chef-backend\\data\\valid_data.csv"
SEASONAL_DATASET_PATH = "C:\\Users\\Aarav\\Desktop\\cheff\\smart-chef-backend\\data\\SeasonalRecipes.csv"
MODEL_PATH = "C:\\Users\\Aarav\\Desktop\\cheff\\smart-chef-backend\\models\\recipe_model.h5"
TOKENIZER_PATH = "C:\\Users\\Aarav\\Desktop\\cheff\\smart-chef-backend\\models\\tokenizer.pkl"
CLASS_NAMES_PATH = "C:\\Users\\Aarav\\Desktop\\cheff\\smart-chef-backend\\class_names.json"
IMAGE_FOLDER = "C:\\Users\\Aarav\\Desktop\\cheff\\smart-chef-backend\\data\\Valid_Food_Images"
    ...
    try:
        ...
        for path in possible_paths:
            if os.path.exists(path):
                file_path = path
                print(f"Found seasonal CSV file at: {path}")
                break
        
        if not file_path:
            print("Seasonal CSV file not found in any of the expected locations")
            return create_sample_seasonal_data()
        ...
        for encoding in encodings_to_try:
            try:
                print(f"Attempting to read seasonal CSV with {encoding} encoding")
                df = pd.read_csv(file_path, encoding=encoding)
                print(f"Successfully loaded {len(df)} seasonal recipes with {encoding} encoding")
                return df
            except Exception as e:
                print(f"Error loading seasonal CSV with {encoding} encoding: {e}")
                continue
        
        # If we've tried all encodings and none worked, create a sample DataFrame
        print("All encoding attempts failed, creating sample seasonal data")
        return create_sample_seasonal_data()
    except Exception as e:
        print(f"Unexpected error loading seasonal CSV: {e}")
        return create_sample_seasonal_data()
...
df = load_dataset()
seasonal_df = load_seasonal_recipes()
...
# Seasonal Recipes API - from seasonal.py
@app.route('/api/seasonal-recipes', methods=['GET'])
def get_seasonal_recipes():
    if seasonal_df.empty:
        return jsonify({"error": "Could not load seasonal recipes"}), 500
    ...
    for season in seasons:
        ...
        season_recipes = seasonal_df[seasonal_df['Season'] == season]
